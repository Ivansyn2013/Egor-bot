...
from acces_reader import (
    db_mysql_all_products,
    db_mysql_category_request,
    db_mysql_request,
)
from create_obj import bot, router
from features import get_answer_str, my_fuzzy_search
from features.answer_and_question import MAIN_MENU_ANSWERS
from handlers.admin import MyFilter
from inline_butn import get_product_list_kb, inline_button_gen
from inline_butn.search_inline_but import MyCallbackData, inline_buttons_gen_category
from keybords import get_inline_search_kb, kb_client, kb_search
...
async def cancel_search_handler(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.reply("Команда отмены: ok", reply_markup=kb_client)
...
async def command_search(message: types.Message):
    ...
    await bot.send_message(
        message.from_user.id, "Поиск продуктов", reply_markup=kb_search
    )
...
async def start_searching(message: types.Message, state: FSMContext):
    ...
    await message.reply(
        text=MAIN_MENU_ANSWERS["start_search"],
...
async def start_category_search(message: types.Message):
    ...
    """
    await message.reply(
        "Категории продуктов", reply_markup=await inline_buttons_gen_category()
    )


async def callback_category(query: types.CallbackQuery, callback_data: MyCallbackData):
    global kb_list
    ...
    await query.answer()
    await query.message.edit_text(f"{category_product_in}", reply_markup=kb_list[0])


async def search_go_to_db(message: types.Message, state: FSMContext):
    if message.text == "Искать":
        await bot.send_message(message.from_user.id, "Введи название продукта:")
    elif message.text == "Выйти из поиска":
        await state.clear()
        await bot.send_message(
            message.from_user.id, "Отмена поиска", reply_markup=kb_client
        )
        ...
        if res is None:
            ...
            search_dict_ready = {
                x: search_dict[x] for x in search_dict if x in search_option_list
            }
...
async def search_callback(
    ...
    async with state.proxy() as data:
        ...
        request = [
            x for x, y in data["bd_dict"].items() if y == int(callback_data["bd_id"])
        ]

        res = await db_mysql_request(request[0])

    if res is None:
        await bot.send_message(query.from_user.id, text="Ничего не найдено")
        await state.clear()
        ...
        await bot.send_photo(
            query.from_user.id, "Пока нет", f"{get_answer_str(res)}", parse_mode="html"
        )
        await state.set_state(FSMSearch.fs_search)
...
async def command_product_list(message: types.Message):
    ...
    if kb_list is None:
        ...
        await bot.send_message(
            message.from_user.id, "Список продуктов", reply_markup=kb_list[0]
        )


# Callback для списка продуктов
#
async def product_list_enter(
    ...
    request = [x for x, y in search_dict.items() if y == int(callback_data.bd_id)]
    res = await db_mysql_request(request[0])
    ...
    try:
        await bot.send_photo(
            ...
            photo=types.BufferedInputFile(
                image_data, filename=res["Название продукта"][0]
            ),
            caption=f"{get_answer_str(res)}",
...
async def product_list_callback_next(
    query: types.CallbackQuery, callback_data: MyCallbackData
):
    global kb_list
    ...
    if kb_number >= len(kb_list) - 1:
        ...
        await query.message.edit_text(
            "Список продуктов", reply_markup=kb_list[kb_number + 1]
        )


async def product_list_callback_back(
    query: types.CallbackQuery, callback_data: MyCallbackData
):
    global kb_list
    kb_number = int(callback_data.kb_number)
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.message.register(start_searching, F.text(equals=["Поиск"], ignore_case=True))
    dp.message.register(start_searching, F.text == "Поиск")
    dp.message.register(command_start, CommandStart())
    dp.message.register(command_product_list, F.text == "Список продуктов")
    dp.message.register(search_go_to_db, FSMSearch.fs_search)
    dp.message.register(
        cancel_search_handler,
        F.text.in_(["Выйти из поиска", "Отмена"]),
    )
    dp.message.register(cancel_search_handler, F.text == "отмена")
    dp.message.register(start_category_search, F.text == "Категории продуктов")
    dp.callback_query.register(
        search_callback,
        cd_data.filter(F.action == "search"),
    )
    dp.callback_query.register(
        product_list_callback_next, cd_data.filter(F.action == "next")
    )
    dp.callback_query.register(
        product_list_callback_back, cd_data.filter(F.action == "back")
    )
    dp.callback_query.register(
        product_list_enter, cd_data.filter(F.action == "search_in")
    )
    dp.callback_query.register(
        callback_category, MyCallbackData.filter(F.action == "category")
    )
