...
async def answer_and_question(message: types.Message):
    ...
    if message.text == "Узнать о боте":
        await message.reply(
            MAIN_MENU_ANSWERS["Start message"], reply_markup=kb_answer_and_qusetion
        )
    if message.text in STR_ANSWER_AND_QUESTION.keys():
        await message.reply(
            f"{STR_ANSWER_AND_QUESTION[message.text]}",
...
async def donat_handler(message: types.Message):
    ...
    await message.answer("Донат", reply_markup=DONATE_KB)
...
async def consultations_handler(message: types.Message):
    await message.reply(MAIN_MENU_ANSWERS[message.text], reply_markup=kb_client)
