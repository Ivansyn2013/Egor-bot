...
from keybords import ADMIN_KB_SENDMESSAGE, admin_kb_check
...
async def ready_to_send_mes_state(message: types.Message, state: FSMContext):
    ...
    await message.reply(
        "Готов к отправке. Следующее сообщение будет отправлено пользователям бота"
    )
...
async def set_description(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await FSMAdmin.next()
        await bot.send_message(message.from_user.id, "Жду категорию:")
...
async def set_category(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await FSMAdmin.next()
        await bot.send_message(message.from_user.id, "Жду fodmap:")
...
async def set_fodmap(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id, "Жду photo:")
...
async def set_search_product_id(message: types.Message, state: FSMContext):
    ...
    if product_id:
        ...
        await bot.send_message(message.from_user.id, "Объект найден. Жду фото:")
        await FSMAdmin.photo.set()
    else:
        await bot.send_message(message.from_user.id, "Не найдено. Выход")
        state.finish()
...
async def set_photo(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        message.from_user.id, "Проверь данные", reply_markup=admin_kb_check
    )
...
async def update_photo(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if request:
            ...
            await bot.send_message(
                data["chat_id"], "Запрос к базе данных вернул ошибку"
            )
            await state.finish()
...
async def update_photo_cancel(state: FSMContext):
    """Not work"""
    async with state.proxy() as data:
        await bot.send_message(data["chat_id"], "Отмена")
        await state.finish()
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.update.register(cancel_handler, F.text(equals="отмена", ignore_case=True))
